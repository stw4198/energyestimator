...
class Estimator():
    ...
    def __init__(self, rat_fn=None, bonsai_fn=None, min_t=-3, max_t=6,
                 dark_noise=3000., qe=0.3, pmt_sep=750., medium=None,
                 geometry=None, load_lib=True):
        self.rat_fn = rat_fn
    ...
    def parse_options(self, argv=None):
        ...
        parser.add_argument("--minT", help="start time of rolling window \
                            ...
                            a negative number or zero", type=int, default=-3)
        ...
        parser.add_argument("--pmtSep", help="specify the average PMT \
                            diagonal separation.", type=float, default=750.)
    ...
    def eventloop(self, write_tree=True):
        ...
        for b_entry, b_event in enumerate(self.bonsai_t):
            ...
            for ev_idx in range(evcount):
                ...
                nXeff = 0.
                for hit in range(nX):
                    #multihit_cor = occupancy(nXPMTpos, hit, nX, self.pmt_sep)
        ...
        if write_tree:
            self.write_to_tree(nXeff_values)
    ...
    def write_to_tree(self, values):
        nXeff_value = np.empty((1), dtype="double")
        branch = self.bonsai_t.Branch("nXeff", nXeff_value, "nXeff/D")
        for value in values:
            nXeff_value[0] = value
            branch.Fill()
